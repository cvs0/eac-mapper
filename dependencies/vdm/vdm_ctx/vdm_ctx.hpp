...
namespace vdm
{
    ...
    std::atomic<bool> is_page_found = false;
    std::atomic<void*> syscall_address = nullptr;
    std::uint16_t nt_page_offset;
    std::uint32_t nt_rva;
    std::uint8_t* ntoskrnl;
    ...
    public:
        ...
        __forceinline auto syscall(T func, Ts... args) const -> std::invoke_result_t<T, Ts...>
        {
            ...
            *reinterpret_cast<void**>(jmp_code + 6) = func;
            ...
            if constexpr (std::invocable<T, Ts...>) {
                auto result = reinterpret_cast<T(__stdcall*)(Ts...)>(proc)(args...);
        ...
        __forceinline auto get_peprocess(std::uint32_t pid) -> PEPROCESS
        {
            ...
            this->syscall<decltype(&PsLookupProcessByProcessId), HANDLE, PEPROCESS*>(
                ps_lookup_peproc,
                (HANDLE)pid,
                &peproc
            );
